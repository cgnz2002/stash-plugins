// JustFor.Fans Metadata Sync -- performer-page button.
//
// Adds a "Sync JustFor.Fans" button to each performer's page that triggers a full
// re-sync scoped to that one performer. The button only needs the performer id;
// the Python "Sync Performer" task maps it back to the JFF username via the
...
(function () {
  ...
  var BTN_ID = "jff-sync-performer-btn";
  ...
  function runSync(performerId, btn) {
    ...
    graphql(mutation, { args: { mode: "performer", performerId: performerId } })
      ...
      .catch(function (err) {
        console.error("[jff-stash-sync] performer sync failed:", err);
        btn.textContent = "Sync failed";
        window.alert(
          "JustFor.Fans sync failed: " + (err && err.message ? err.message : err)
        );
      })
      .finally(function () {
        setTimeout(function () {
  ...
  function makeButton() {
    ...
    btn.textContent = "Sync JustFor.Fans";
