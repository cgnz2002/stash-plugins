"""Turn JustFor.Fans post text and media rows into Stash metadata.

Shared with of-stash-sync (same title/details, emoji-aware truncation, tag
matching and title-exclusion handling); the only platform difference is that a
collaborator credited by a bare profile link is matched on ``justfor.fans/<user>``
instead of ``onlyfans.com/<user>``. Standard library only.
"""
...
# Profile links to a collaborator, e.g. justfor.fans/ChicagoNerd (with or without
# scheme/www/trailing punctuation). Some creators credit a collaborator with a
# bare profile URL instead of an @mention, so these are treated as mentions too.
# On JustFor.Fans the first path segment IS the creator's username -- a post link
# is justfor.fans/<username>?Post=<key> -- so the segment is taken as-is (the
# query string is not part of the capture). Purely numeric captures are still
# skipped in parse_mentions as a cheap guard against non-profile paths.
_PROFILE_URL_RE = re.compile(
    r"justfor\.fans/([A-Za-z0-9_\.\-]+)", re.IGNORECASE
)
...
class MediaProcessor:
    ...
    def parse_mentions(self, text):
        ...
        (`justfor.fans/<username>`), since some creators link a collaborator by
        URL instead of an @mention. On JustFor.Fans the first path segment is the
        username (a post link is `justfor.fans/<username>?Post=<key>`), so it is
        taken as-is; purely-numeric captures are skipped as a guard against
        non-profile paths. A creator linking their own post just re-credits
        themselves, which is harmless (they are already a performer).
        """
        mentions = []
        ...
        for match in _PROFILE_URL_RE.findall(text):
            ...
            # Guard against a non-profile numeric path segment.
